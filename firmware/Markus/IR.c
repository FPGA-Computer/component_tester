...
void IR_Detector(void)
{
   ...
   */
    ...
    #ifdef SW_IR_ALT_PIN
    /* set probes: probe-1 -- Vcc / probe-2 -- Gnd / probe-3 (HiZ) -- Rh -- Gnd */
    ADC_PORT = (1 << TP1);                /* pull down probe-1, pull up probe-2 */
    #else
    ...
    #ifdef SW_IR_ALT_PIN
    /* set probes: probe-1 -- Rl -- Vcc / probe-2 -- Gnd / probe-3 (HiZ) -- Rh -- Gnd */
    ADC_DDR = (1 << TP2);                 /* probe-2 */
    /* pull up probe-1 via Rl, pull down probe-3 via Rh */
    R_DDR = (1 << R_RL_1) | (1 << R_RH_3);     /* enable resistors */
    R_PORT = (1 << R_RL_1);                    /* pull up probe-1, pull down probe-3 */
    #else
